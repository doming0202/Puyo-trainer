...
function releaseMemberFocus(room, member, notify = true) {
  ...
  for (let index = 0; index < room.focus.length; index += 1) {
    if (room.focus[index] === member.id) { room.focus[index] = null; changed = true }
  }
...
function allowedRoomAction(action) {
  return new Set(['left', 'right', 'rotate-left', 'rotate-right', 'soft-drop', 'hard-drop', 'reset-turn', 'undo', 'redo', 'toggle-player-pause']).has(action)
}
function validTimeState(state) {
  if (!state || typeof state !== 'object' || !validPlayerIndex(state.playerIndex)) return false
  if (!state.player || typeof state.player !== 'object') return false
  return Number.isFinite(state.tick) && Number.isFinite(state.elapsedMs)
}
function mergePlayer(room, state) {
  const playerIndex = Number(state.playerIndex)
  if (!room.state?.game?.players?.[playerIndex]) return
  const players = [...room.state.game.players]
  players[playerIndex] = { ...players[playerIndex], ...state.player }
  room.state = {
    ...room.state,
    game: {
      ...room.state.game,
...
function removeMember(socket) {
  ...
  if (room.members.length === 0) {
    ...
    broadcast(room, { type: 'presence', studentCount: room.members.filter(item => item.role === 'student').length })
  }
  socket.room = null
  socket.member = null
}
...
function handleMessage(socket, raw) {
  ...
  if (message.type === 'join-room') {
    ...
    socket.member = member
    send(socket, {
      type: 'room-joined', roomId: room.id, role, memberId: member.id,
    ...
    broadcast(room, { type: 'presence', studentCount: room.members.filter(member => member.role === 'student').length }, socket)
  ...
  if (message.type === 'request-focus') {
    ...
    if (!validPlayerIndex(playerIndex)) return send(socket, { type: 'focus-denied', playerIndex: null, reason: 'invalid-player' })
    const owner = room.focus[playerIndex]
    if (owner && owner !== member.id) {
      return send(socket, { type: 'focus-denied', playerIndex, reason: 'occupied', ownerRole: room.members.find(item => item.id === owner)?.role || null })
    }
  ...
  if (message.type === 'release-focus') {
    const playerIndex = Number(message.playerIndex)
    if (validPlayerIndex(playerIndex) && room.focus[playerIndex] === member.id) {
  ...
  if (message.type === 'reset-state') {
    ...
    room.state = state
    room.liveState = null
    broadcast(room, { type: 'reset-state', state }, socket)
    return
  }
  ...
  if (message.type === 'student-action') {
    ...
    if (!validPlayerIndex(playerIndex) || !allowedRoomAction(action) && action !== 'global-pause') {
...
const server = createServer((request, response) => {
  ...
  if (!candidate.startsWith(DIST + sep) && candidate !== DIST) { response.writeHead(403); response.end(); return }
  let file = candidate
  if (!existsSync(file) || !statSync(file).isFile()) file = join(DIST, 'index.html')
  try {
    const content = readFileSync(file)
    const ext = extname(file)
    const types = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.svg': 'image/svg+xml', '.json': 'application/json; charset=utf-8', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp', '.mp3': 'audio/mpeg', '.ico': 'image/x-icon' }
...
wss.on('connection', socket => {
  ...
  socket.on('close', () => removeMember(socket))
  socket.on('error', () => removeMember(socket))
})

setInterval(() => {
  const cutoff = Date.now() - ROOM_TTL_MS
  for (const [id, room] of rooms) if (room.members.length === 0 || room.lastActiveAt < cutoff) rooms.delete(id)
}, 10 * 60 * 1000).unref()
