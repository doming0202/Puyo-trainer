...
type ResetRoomDetail = SharedRoomState
...
function dispatchFocusState(focus: RoomFocusState | null, memberId: string | null, connected: boolean): void { window.dispatchEvent(new CustomEvent('puyo-room-focus-state', { detail: { focus, memberId, connected } })) }
async function copyText(text: string): Promise<void> { if (navigator.clipboard?.writeText) { await navigator.clipboard.writeText(text); return }; const textarea = document.createElement('textarea'); textarea.value = text; textarea.setAttribute('readonly', ''); textarea.style.position = 'fixed'; textarea.style.opacity = '0'; document.body.appendChild(textarea); textarea.select(); try { if (!document.execCommand('copy')) throw new Error('copy-failed') } finally { textarea.remove() } }

export function RoomPanel({ open, onClose, game, replay, currentTimelineMs, onRemoteState, onRemoteLiveState }: { open: boolean; onClose: () => void; game: GameState; replay: ReplayState; currentTimelineMs: number; onRemoteState: (state: SharedRoomState) => void; onRemoteLiveState: (state: SharedRoomLiveState) => void }) {
  const clientRef = useRef<RoomClient | null>(null)
  const gameRef = useRef(game); gameRef.current = game
  const replayRef = useRef(replay); replayRef.current = replay
  const timelineRef = useRef(currentTimelineMs); timelineRef.current = currentTimelineMs
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('puyo-room-request-focus', onFocusRequest); window.removeEventListener('puyo-room-release-focus', onFocusRelease); window.removeEventListener('puyo-room-local-action', onLocalAction); window.removeEventListener('puyo-room-reset', onLocalReset); unsubscribe(); client.disconnect(); clientRef.current = null; dispatchFocusState(null, null, false) }
