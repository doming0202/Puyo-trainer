...
import type { RoomPlayerState, RoomTimeStateSync, SharedRoomLiveState, SharedRoomState, RoomRole, RoomFocusState } from '../game/room-client'
import { getRoomInviteFromUrl, getStoredRoomSession, RoomClient } from '../game/room-client'
import { setRoomAccessRole } from '../room-access-policy'
...
function compactPlayerState(player: PlayerState): RoomPlayerState {
...
function dispatchFocusState(focus: RoomFocusState | null, memberId: string | null, connected: boolean, role: RoomRole | null): void {
  ...
  window.dispatchEvent(new CustomEvent('puyo-room-focus-state', { detail: { focus, memberId, connected, role } }))
}
...
export function RoomPanel({ open, onClose, game, replay, currentTimelineMs, onRemoteState, onRemoteLiveState }: { open: boolean; onClose: () => void; game: GameState; replay: ReplayState; currentTimelineMs: number; onRemoteState: (state: SharedRoomState) => void; onRemoteLiveState: (state: SharedRoomLiveState) => void }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = client.subscribe(message => {
      ...
      else if (message.type === 'focus-state') dispatchFocusState(message.focus, client.memberId || null, true, client.role)
      else if (message.type === 'focus-granted') { dispatchFocusState(message.focus, client.memberId || null, true, client.role); window.dispatchEvent(new CustomEvent('puyo-room-focus-granted', { detail: { playerIndex: message.playerIndex } })) }
      ...
      else if (message.type === 'disconnected') { setRole(null); setStudentCount(0); setStatus('接続が切れました'); setStoredRoom(getStoredRoomSession()); dispatchFocusState(null, null, false, null) }
