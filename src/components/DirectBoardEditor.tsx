...
export function DirectBoardEditor({ board, onBoardChange, onPairEdit }: { board: Board; onBoardChange: (board: Board) => void; onPairEdit: (pair: Pair) => void }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Delete' && selected.size) { e.preventDefault(); const next = board.map(r => [...r]); selected.forEach(k => { const [x,y] = k.split(',').map(Number); next[y][x] = null }); onBoardChange(next); setSelected(new Set()); setMenu(null) } }
  ...
  const select = (p: Point, e: MouseEvent) => {
    setMenu(null)
    if (e.shiftKey && anchor) { setSelected(new Set(rangeKeys(anchor,p))); return }
    if (e.ctrlKey || e.metaKey) { setSelected(cur => { const n = new Set(cur); const k = keyOf(p); n.has(k) ? n.delete(k) : n.add(k); return n }); setAnchor(p); return }
    setSelected(new Set([keyOf(p)])); setAnchor(p)
  }
