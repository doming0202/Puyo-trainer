...
export function randomPair(): Pair {
  return { axis: randomColor(), child: randomColor() }
}
...
export function createGame(): GameState {
  return {
    players: [createPlayer('human'), createPlayer('fixed')],
    activePlayer: 0,
    running: true,
    tick: 0,
  }
...
export function spawnPair(pair: Pair): ActivePair {
  return { pair, x: 2, y: 1, rotation: 0 }
}

const OFFSETS: Record<Rotation, readonly [number, number]> = {
  0: [0, -1],
  1: [1, 0],
  2: [0, 1],
  3: [-1, 0],
...
export function canPlace(board: Board, pair: ActivePair): boolean {
  return cellsOf(pair).every(({ x, y }) => x >= 0 && x < COLS && y >= 0 && y < ROWS && board[y][x] === null)
}
