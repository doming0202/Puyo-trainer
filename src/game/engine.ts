...
export function movePair(player: PlayerState, dx: number): PlayerState {
  ...
  if (!canPlace(player.board, candidate)) return player
...
export function rotatePair(player: PlayerState, direction: 1 | -1): PlayerState {
  ...
  if (!canPlace(player.board, quickCandidate)) return { ...player, quickTurnArmed: false }
...
export function stepDown(player: PlayerState): PlayerState {
  ...
  if (!canPlace(player.board, candidate)) {
...
export function hardDrop(player: PlayerState): PlayerState {
  ...
  while (canPlace(player.board, { ...current, y: current.y + 1 })) current = { ...current, y: current.y + 1 }
...
function beginPlacement(player: PlayerState): PlayerState {
  ...
  for (const { x, y, color } of cellsOf(player.current)) if (y >= 0 && y < ROWS && x >= 0 && x < COLS) board[y][x] = color
