...
export const COLORS: PuyoColor[] = [1, 2, 3, 5, 4]
...
export function createPlayer(controlMode: PlayerState['controlMode'] = 'human'): PlayerState {
  ...
  const base: Omit<PlayerState, 'turnStart' | 'undoStack' | 'redoStack'> = {
    board: emptyBoard(), hidden: emptyHiddenBoard(), current: spawnPair(first),
    next: [randomPair(), randomPair(), randomPair(), randomPair()], garbage: 0, score: 0, chain: 0,
    controlMode, alive: true, fallElapsedMs: 0, lockElapsedMs: 0, quickTurnArmed: false,
  }
...
/** Spawn just above the visible field. The axis starts on the first visible row and the child is allowed to begin in the hidden 13th row. */
...
export function rotatePair(player: PlayerState, direction: 1 | -1): PlayerState {
  ...
  const candidates = [
    { ...player.current, rotation }, { ...player.current, rotation, x: player.current.x - 1 }, { ...player.current, rotation, x: player.current.x + 1 },
  ]
  if (player.current.y === 0) candidates.push(
    { ...player.current, rotation, y: -1 }, { ...player.current, rotation, x: player.current.x - 1, y: -1 }, { ...player.current, rotation, x: player.current.x + 1, y: -1 },
  )
...
export function advancePlayer(player: PlayerState, deltaMs: number): PlayerState {
  ...
  const candidate = { ...player.current, y: player.current.y + 1 }
  if (!canPlace(player, candidate)) { const lockElapsedMs = player.lockElapsedMs + delta; if (lockElapsedMs >= LOCK_DELAY_MS) return beginPlacement(player); return { ...player, lockElapsedMs } }
  ...
  return { ...player, current: candidate, fallElapsedMs: fallElapsedMs - FALL_INTERVAL_MS, lockElapsedMs: 0, quickTurnArmed: false }
}
...
function beginPlacement(player: PlayerState): PlayerState {
  const board = player.board.map((row) => [...row]), hidden = player.hidden.map((row) => [...row])
...
function applyGravityWithOrigins(board: Board, hidden: HiddenBoard): { board: Board; hidden: HiddenBoard; fallingCells: FallingCell[] } {
  ...
  for (let x = 0; x < COLS; x += 1) {
    ...
    occupied.forEach(({ y: fromCombinedY, color }, index) => {
      ...
      const fromY = fromCombinedY - HIDDEN_ROWS, targetY = targetCombinedY - HIDDEN_ROWS
...
function finishPlacement(player: PlayerState): PlayerState {
  const nextPair = player.next[0] ?? randomPair(), next = [...player.next.slice(1), randomPair()], nextCurrent = spawnPair(nextPair)
...
function findGroups(board: Board): Array<Array<{ x: number; y: number }>> {
  const seen = new Set<string>(), groups: Array<Array<{ x: number; y: number }>> = []
  for (let y = 0; y < ROWS; y += 1) for (let x = 0; x < COLS; x += 1) {
    const color = board[y][x], key = `${x},${y}`
    if (color === null || seen.has(key)) continue
    const queue = [{ x, y }], group: Array<{ x: number; y: number }> = []; seen.add(key)
    while (queue.length) {
      ...
      for (const [nx, ny] of [[current.x + 1, current.y], [current.x - 1, current.y], [current.x, current.y + 1], [current.x, current.y - 1]]) {
        ...
        const neighborKey = `${nx},${ny}`; if (seen.has(neighborKey)) continue; seen.add(neighborKey); queue.push({ x: nx, y: ny })
      }
    }
    groups.push(group)
  }
