...
export default function App() {
  ...
  const copyOpponentTurn = () => {
    ...
    if (!source.alive || source.resolution || !target.alive || target.resolution || editMode || replayRef.current.playing) {
      setMessage('現在の状態では相手の手番をコピーできません')
      return
    }
    ...
    setGame(nextGame)
    setReplay(state => appendFrame(state, nextGame, elapsedMs))
    setMessage(`Player ${sourceIndex === 0 ? 'A' : 'B'} の手番を Player ${targetIndex === 0 ? 'A' : 'B'} にコピーしました`)
  }
  ...
  const restoreTurnCopy = () => {
    ...
    setMessage('相手の手番をコピーする前の状態に戻しました')
  }
  ...
  const canCopyTurn = !editMode && !replay.playing && game.players[focusedPlayer].alive && !game.players[focusedPlayer].resolution && game.players[focusedPlayer].controlMode === 'human' && game.players[focusedPlayer === 0 ? 1 : 0].alive && !game.players[focusedPlayer === 0 ? 1 : 0].resolution
  ...
  <section className="arena"><PlayerPanel title="A" player={game.players[0]} focused={focusedPlayer===0} onFocus={()=>focusPlayer(0)} onMode={mode=>setMode(0,mode)} nextVisible={nextVisible} editMode={editMode&&editPlayer===0} onBoardChange={setEditedBoard} onPairEdit={setEditedPair} editableControls={editableControls}/><div className="vs"><span>VS</span><small>LOCAL</small><span className="turn-copy-focus">FOCUS {focusedLabel} · 相手 {opponentLabel}</span><button className="board-edit-button" disabled={!canCopyTurn} onClick={copyOpponentTurn}>相手の手番をコピー</button><button className="board-edit-button" disabled={!turnCopyBackup} onClick={restoreTurnCopy} style={!turnCopyBackup ? { opacity: 0.45, cursor: 'default' } : undefined}>コピー前に戻す</button><button className="board-edit-button" onClick={() => setEditMode(v=>!v)}>{editMode ? '編集終了' : '盤面を直接編集'}</button></div><PlayerPanel title="B" player={game.players[1]} focused={focusedPlayer===1} onFocus={()=>focusPlayer(1)} onMode={mode=>setMode(1,mode)} nextVisible={nextVisible} editMode={editMode&&editPlayer===1} onBoardChange={setEditedBoard} onPairEdit={setEditedPair} editableControls={editableControls}/></section>
  ...
  <section className="controls"><div><strong>フォーカス中のPlayer</strong>　左 / 右 / 回転 / 落下 / ハードドロップを共通キーバインドで操作</div><div><strong>Ctrl+1</strong> Player A　<strong>Ctrl+2</strong> Player B　・　盤面クリックでも切替</div><div><strong>F</strong> ゲーム再開 / 停止　<strong>C</strong> 編集モード切替　<strong>⚙️</strong> キーバインド設定</div><div><strong>相手の手番をコピー</strong>　フォーカス中Playerに相手の現在の組ぷよをコピー · <strong>コピー前に戻す</strong> で操作前へ復元</div></section><footer>独自ゲームエンジン · 公式素材・データ不使用 · Phase 4 Position Editor</footer>{keybindModalOpen && <KeybindModal keybinds={keybinds} onChange={setKeybinds} onClose={()=>setKeybindModalOpen(false)} />}</main>
}
