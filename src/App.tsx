...
export default function App() {
  ...
  const reset = () => { const next = createGame(); setGame(next); setReplay(createReplay(next)); setMessage(''); setEditMode(false); focusPlayer(0); resetClock(0) }
  ...
  const loadSnapshot = (snapshot:Snapshot) => { const restored = cloneGameState(snapshot.state); setGame(restored); setReplay(createReplay(restored)); setLibraryOpen(false); setEditMode(false); setMessage(`「${snapshot.title}」を読み込みました`); resetClock(0); setFocusedPlayer(restored.activePlayer) }
  ...
  <section className="arena"><PlayerPanel title="A" player={game.players[0]} focused={focusedPlayer===0} onFocus={()=>focusPlayer(0)} onMode={mode=>setMode(0,mode)} nextVisible={nextVisible} editMode={editMode&&editPlayer===0} onBoardChange={setEditedBoard} onPairEdit={setEditedPair} editableControls={editableControls}/><div className="vs"><span>VS</span><small>LOCAL</small><button className="board-edit-button" onClick={() => setEditMode(v=>!v)}>{editMode ? '編集終了' : '盤面を直接編集'}</button></div><PlayerPanel title="B" player={game.players[1]} focused={focusedPlayer===1} onFocus={()=>focusPlayer(1)} onMode={mode=>setMode(1,mode)} nextVisible={nextVisible} editMode={editMode&&editPlayer===1} onBoardChange={setEditedBoard} onPairEdit={setEditedPair} editableControls={editableControls}/></section>
  ...
  <section className="controls"><div><strong>フォーカス中のPlayer</strong>　左 / 右 / 回転 / 落下 / ハードドロップを共通キーバインドで操作</div><div><strong>Ctrl+1</strong> Player A　<strong>Ctrl+2</strong> Player B　・　盤面クリックでも切替</div><div><strong>F</strong> ゲーム再開 / 停止　<strong>C</strong> 編集モード切替　<strong>キーバインド</strong> 設定は別モーダル</div></section><footer>独自ゲームエンジン · 公式素材・データ不使用 · Phase 4 Position Editor</footer>{keybindModalOpen && <KeybindModal keybinds={keybinds} onChange={setKeybinds} onClose={()=>setKeybindModalOpen(false)} />}</main>
}
