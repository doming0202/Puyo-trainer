...
import { appendFrame, createReplay, findFrameAtElapsed, frameToGame, moveCursor, REPLAY_SPEEDS, type ReplayState } from './game/replay'
...
function formatTime(ms: number): string { const totalSeconds = Math.max(0, ms) / 1000; const minutes = Math.floor(totalSeconds / 60); const seconds = (totalSeconds % 60).toFixed(1).padStart(4, '0'); return `${minutes}:${seconds}` }
...
export default function App() {
  ...
  const dispatch = useCallback((playerIndex: 0 | 1, action: Parameters<typeof updatePlayer>[1]) => {
    setGame(current => {
      ...
      if (editMode || !player.alive || player.controlMode !== 'human' || !current.running) return current
      ...
      setReplay(r => appendFrame(r, nextGame, elapsedMs))
      return nextGame
    })
  ...
  }, [dispatch, editMode, syncElapsed])

  useEffect(() => { const timer = window.setInterval(() => setGame(current => { if (!current.running || editMode || replaySelected) return current; const elapsedMs = syncElapsed(current.running); const players = [...current.players] as [PlayerState, PlayerState]; players.forEach((player, index) => { if (player.controlMode === 'human' && player.alive && !player.resolution) players[index] = updatePlayer(player, 'soft-drop') }); const nextGame = { ...current, players, tick: current.tick + 1 }; setReplay(r => appendFrame(r, nextGame, elapsedMs)); return nextGame }), 900); return () => window.clearInterval(timer) }, [editMode, replaySelected, syncElapsed])
  useEffect(() => { const timer = window.setInterval(() => setGame(current => { if (!current.running || editMode || replaySelected || !current.players.some(player => player.resolution)) return current; const elapsedMs = syncElapsed(current.running); const players = [...current.players] as [PlayerState, PlayerState]; players.forEach((player,index) => { if (player.resolution) players[index] = advanceResolution(player) }); const nextGame = { ...current, players, tick: current.tick + 1 }; setReplay(r => appendFrame(r, nextGame, elapsedMs)); return nextGame }), 420); return () => window.clearInterval(timer) }, [editMode, replaySelected, syncElapsed])
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      setReplay(current => ({ ...current, cursor, playing: cursor < current.frames.length - 1 }))
    }, 40)
    return () => window.clearInterval(timer)
  ...
  const seek = (cursor:number) => {
    const clamped = Math.max(0, Math.min(replayRef.current.frames.length - 1, cursor))
    const frame = replayRef.current.frames[clamped]
  ...
  const togglePlayback = () => {
    ...
    if (currentReplay.cursor >= currentReplay.frames.length - 1) {
      const frame = currentReplay.frames[0]
      resetClock(frame.elapsedMs)
      setGame(frameToGame(frame, false))
      setReplay(current => ({ ...current, cursor: 0, playing: true }))
      return
    }
    setGame(current => ({ ...current, running: false }))
    setReplay(current => ({ ...current, playing: true }))
  }
  ...
  const timelinePlayingLabel = replay.playing ? '⏸' : '▶'
  ...
  <section className="arena"><PlayerPanel title="A" player={game.players[0]} onMode={mode=>setMode(0,mode)} nextVisible={nextVisible} editMode={editMode&&editPlayer===0} onBoardChange={setEditedBoard} onPairEdit={setEditedPair} editableControls={{changeCurrentColor,rotateCurrent,changeNext,changeGarbage,setGarbageValue}}/><div className="vs"><span>VS</span><small>LOCAL</small><button className="board-edit-button" onClick={() => setEditMode(v=>!v)}>{editMode ? '編集終了' : '盤面を直接編集'}</button></div><PlayerPanel title="B" player={game.players[1]} onMode={mode=>setMode(1,mode)} nextVisible={nextVisible} editMode={editMode&&editPlayer===1} onBoardChange={setEditedBoard} onPairEdit={setEditedPair} editableControls={{changeCurrentColor,rotateCurrent,changeNext,changeGarbage,setGarbageValue}}/></section>
  <section className="replay-panel"><div className="replay-header"><div><div className="aside-label">TIMELINE</div><strong>{formatTime(currentTimelineMs)} / {formatTime(timelineMaxMs)}</strong></div><span>Frame {replay.cursor+1} / {replay.frames.length}</span></div><div className="timeline-wrap"><div className="timeline-labels"><span>0:00.0</span><span>{formatTime(currentTimelineMs)}</span><span>{game.running && replay.cursor === lastCursor ? 'LIVE' : formatTime(lastFrame?.elapsedMs ?? 0)}</span></div><input className="timeline" type="range" min="0" max={Math.max(1,timelineMaxMs)} step="10" value={Math.min(currentTimelineMs,Math.max(1,timelineMaxMs))} onChange={e=>seekTime(Number(e.target.value))} disabled={replay.frames.length < 2}/></div><div className="replay-controls"><button onClick={()=>seek(0)} disabled={replay.frames.length < 2}>⏮</button><button onClick={()=>seek(replay.cursor-1)} disabled={replay.cursor <= 0}>◀</button><button className="play" onClick={togglePlayback} disabled={replay.frames.length < 2}>{timelinePlayingLabel}</button><button onClick={()=>seek(replay.cursor+1)} disabled={replay.cursor >= lastCursor}>▶</button><button onClick={()=>seek(lastCursor)} disabled={replay.frames.length < 2}>⏭</button><div className="speed-buttons">{REPLAY_SPEEDS.map(speed=><button className={replay.speed===speed?'selected':''} key={speed} onClick={()=>setReplay(r=>({...r,speed}))}>{speed}x</button>)}</div></div><div className="replay-hint">タイムラインは常時利用可能 · プレイ中でもシーク可能 · 戻した地点からFキーで再開 · Replay選択時は自動再生</div></section>
